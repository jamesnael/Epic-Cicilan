...
class AjbController extends Controller
{
    ...
    public function update(Request $request, Booking $ajb)
    {
        ...
        try {
            ...
            if ($request->hasFile('dokumen_akhir')) {
                ...
                try {
                    ...
                    \Log::error(json_encode([
                        'action' => 'Send Email Reminder AJB',
                        'data' => $ajb,
                        'error' => $e->getMessage()
                    ], JSON_PRETTY_PRINT));
                }
            ...
            if ($ajb->ajb) {
                $data = AkteJualBeli::where('booking_id', $request->booking_id)->update($request->only(['booking_id', 'ajb_date', 'ajb_time', 'location', 'address','ajb_doc_file_name','ajb_doc_sign_file_name', 'approval_client_status', 'approval_notaris_status', 'approval_developer_status', 'ajb_number', 'upload_date', 'notaris_name']));
                activity()
                 ->performedOn($ajb)
                 ->causedBy(\Auth::user())
