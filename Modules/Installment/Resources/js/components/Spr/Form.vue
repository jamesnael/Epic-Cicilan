	...
	export default {
        ...
        methods: {
    		            ...
    		            .then(response => {
    		            	if (response.data.success) {
    		            		...
    		            		this.form_data = {
    		            			...
    		            			spr: this.moneyFormat(data.spr),
    		            			spr_time: data.spr_time,
                                ...
                                _.forEach(data.payments, (value, key) => {
                                        arr_spr.push({
                                            ...
                                            spr: this.moneyFormat(value.spr),
                                            credit: this.moneyFormat(value.credit)
                                        })
                                });

                                this.unit_spr = arr_spr
            ...
            updatespr() {
                ...
                data.append('unit_spr', JSON.stringify(this.unit_spr))
        	...
        	regeneratespr () {
                let new_spr = []
                let credit = _.toString(this.unit_spr[0].credit).split('.').join('')
                let unit_price = parseInt(credit)

                _.forEach(this.unit_spr, (value, key) => {
                    if (key == 0) {
                        new_spr.push({
                            id: value.id,
                            payment: value.payment,
                            due_date: value.due_date,
                            spr: this.moneyFormat(parseInt(_.toString(value.spr).split('.').join(''))),
                            credit: this.moneyFormat(parseInt(_.toString(value.credit).split('.').join('')))
                        })
                    } else {
                        unit_price = unit_price - parseInt(_.toString(value.spr).split('.').join(''))
                        new_spr.push({
                            id: value.id,
                            payment: value.payment,
                            due_date: value.due_date,
                            spr: key == this.form_data.spr_time ? this.moneyFormat(parseInt(_.toString(value.spr).split('.').join('')) + unit_price) : this.moneyFormat(parseInt(_.toString(value.spr).split('.').join(''))),
                            credit: key == this.form_data.spr_time ? 0 : this.moneyFormat(unit_price)
                        })
                    }
                });
                this.unit_spr = new_spr
            },
