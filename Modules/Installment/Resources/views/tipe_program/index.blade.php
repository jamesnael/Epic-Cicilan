...
@push('table_slot')
    <template v-slot:item.harga_termasuk="{ item }">
    	<ul>
    		<li v-for="el in item.harga_termasuk">@{{ el }}</li>
    	</ul>
	</template>
    <template v-slot:item.harga_tidak_termasuk="{ item }">
    	<ul>
    		<li v-for="el in item.harga_tidak_termasuk">@{{ el }}</li>
    	</ul>
	</template>
@endpush
...
@section('content')
	...
	<table-layout inline-template
		...
		page-text-locale="id"
		add-new-uri="{{ route('tipe-program.create') }}"
		add-new-text="Tambah"
		add-new-color="light-blue lighten-2"
		edit-uri="tipe-program.edit"
		edit-uri-parameter="slug"
		edit-text="Ubah"
		delete-uri="tipe-program.destroy"
